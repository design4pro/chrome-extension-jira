...
const fromStorage = <V>(value: string | null): V | null => (value !== null ? JSON.parse(value) : null);

export const useStorageListener = <V>(key: string, onChange: (newValue: V) => void): void => {
...
export const useStorageWriter = <S>(storage: Storage, key: string, state: S): Error | undefined => {
...
export const useStorageReader = <S>(storage: Storage, key: string, defaultValue: S): S =>
    useMemo(() => {
